...
def Mols2FPs_RDK(mols, molWriter=None):
    for molA in mols:
        ...
        molA.SetProp(
            "_Name",
            (molA.GetProp("_Name") if molA.HasProp("_Name") else "") + "\t" + fpstxt,
        )
        count = 0
...
def GenerateConformations(mol, nconf, ffname, optiters, etol):
    ...
    verbose = (
        ...
        else 1 if logging.getLogger().getEffectiveLevel() == logging.INFO else 0
    )
    ...
    for confId in confIds:
        ...
        if ffname.upper() == "MMFF":
            ff = rdkit.Chem.AllChem.MMFFGetMoleculeForceField(
                molh,
                rdkit.Chem.AllChem.MMFFGetMoleculeProperties(molh, "MMFF94", verbose),
                confId=confId,
            )
        else:
        ...
        try:
            rmsd = rdkit.Chem.AllChem.AlignMol(
                molh, molh_0, prbCid=confId, refCid=confId
            )
        except Exception as e:
            rmsd = 0.0
            logging.info("%s" % str(e))
        logging.debug(
            "\t%3d. RMSD = %.2f ; Ei = %s, Ef = %s, converged = %s"
            % (
                i_conf,
                rmsd,
                ("%.2f" % e_i if e_i else "None"),
                ("%.2f" % e_f if e_f else "None"),
                str(ok_opts[confId]),
            )
        )
    if not confIds:
